...
export function useLeads() {
  ...
  useSalesDataAutoRefresh(fetchLeads, ["all", "leads", "follow-ups", "dashboard", "pipeline"]);
  ...
  const logActivity = async (
    ...
    await fetchLeads();
    notifySalesDataChanged(["leads", "follow-ups", "dashboard", "pipeline"], "lead:log-activity");
  };
  ...
  const archiveLead = async (id: string) => {
    ...
    await supabase
      .from("leads")
      .update({ status: "archived" as Database["public"]["Enums"]["lead_status"] })
      .eq("id", id);
    ...
    await fetchLeads();
    notifySalesDataChanged(["leads", "follow-ups", "dashboard", "pipeline"], "lead:archive");
  };
  ...
  const updateLead = async (id: string, updates: Record<string, unknown>) => {
    ...
    await fetchLeads();
    notifySalesDataChanged(["leads", "follow-ups", "dashboard", "pipeline"], "lead:update");
  };
